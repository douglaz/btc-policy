...
A staged Carrier whose holder decision is not yet terminal. It remains safety state until that decision becomes terminal, its own retirement stopwatch finishes, or the Vault node dies; an exact authenticated and quota-admitted long-lived clock-refused receipt gets a fixed 30-second retry while actionable, whereas terminal or nonmatching stale input keeps its freshness refusal. One-second retry is reserved for a short owner-in-flight or KDF-busy state.
_Avoid_: ruled carrier state, live memo, pending spend

**Carrier retirement stopwatch**:
The process-local monotonic deadline computed once at channel-mode Spend-nonce acceptance from the signed lifetime remaining on that node's accepted clock sample, using the production-fixed, test-pinnable `HotClock`. It remains on the nonce tombstone until it lapses and controls state retention, while signed expiry still controls whether a new holder may count. Later wall-clock changes, retries, relays, Carrier retirement and alternate signatures cannot move it; a clock already wrong at acceptance remains an explicit residual. A node built without the Node channel has no Carrier/stopwatch and keeps ordinary wall-only nonce lifetime.
