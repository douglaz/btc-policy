...
- **`btc-policy-5ag`** — the tracking parent has NOT closed, so no stage's mainnet rung is
  authorized. What exists under it: the vault seals a `network` (manifest revision 2), every
  node checks its backend is on that chain, and `btc-policy-descriptor-network-kind-x00` binds
  the hot/Escape extended-key FLAVOUR to it at assemble, finalize and node load.
