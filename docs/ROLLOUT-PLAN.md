...
|---|---|---|---|---|
| **1** | one machine | open | signet | Path suite on signet for the first time. **Freeze (protocol core)** — the freeze only; external review #1 was REMOVED by [ADR-0017](adr/0017-one-external-review-at-stage-9.md). Prerequisite: the negative controls — SATISFIED IN FULL and no longer blocking. `9yf` (closed 2026-08-09) proved the launch-gate JOB reports a deliberately introduced regression, but its control stopped at step 9 so `attack all` never ran; `nia` (closed 2026-08-10) covered that residue and showed the harness both able to go red on an injected fault and blind to a second one. Neither gates this freeze now. The coverage hole `nia` found is `btc-policy-u98` (P2) and is deliberately NOT a blocker — [ADR-0017](adr/0017-one-external-review-at-stage-9.md) owns that reasoning and the measurements. |
