...
- **A request whose signed `policy_version` is not the node's baked-at-setup configuration is rejected** (`PSBT_INCONSISTENT`, `check = "policy_version"`) — the same broad signed-request/transaction inconsistency category as the two rows above, and NOT an instruction to recompose anything. The check runs inside the coordinator-auth gate, so it fires **before** the freshness nonce is recorded and before any PIN, carrier, KDF, claim, intent, holder, candidate, confirmation, preflight or coordinator-request-memo state moves: a mismatch consumes nothing and the same nonce stays usable. On the two DIRECT routes only terminal Lockdown (`FRAUD_SUSPECTED`, ADR-0008) short-circuits ahead of it; otherwise it is the first coordinator-request validation `/sign` and `/refresh` run, while the fresh-relay and outer-stale `/channel` routes run it before their own receipt-state mutation. Direct `/sign` and `/refresh` return the typed refusal; a peer-relayed copy is silent (`/channel` maps every decodable POLICY outcome to ACCEPTED, which is why the outer-stale envelope answer stays visible) and derives no receipt state; an outer-stale copy stays stale. Envelope replay nonce, peer high-water, and per-peer quota are charged before the opaque request is decoded and stay outside this ordering rule.
- A client that keeps only the CODE must report a generic request/policy inconsistency: `PSBT_INCONSISTENT` never carries retry or recompose advice, and ingress keeps discarding a peer's own `check`/`detail`, so only the direct caller's locally authored `check` disambiguates which inconsistency it was.
