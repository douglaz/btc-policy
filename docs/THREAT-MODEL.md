...
| Chain backend RPC | Per-node bitcoind | Requires `-txindex=1`. Node fails closed if unreadable. At startup it must also report the vault's SEALED chain — and on signet, the default PUBLIC `signet_challenge` — before any other backend check, because a wrong-chain backend answers every later query completely and wrongly: no vault coins, no escape coverage, broadcasts into another mempool. Residual: this reads what Core SAYS (no genesis or work comparison), so two regtest instances and a fork still reporting `chain:"main"` are indistinguishable, and it runs once rather than per tick. |
